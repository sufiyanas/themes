...
class DropDownItems<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const DropDownItems({
...
/// takes a list of [DropDownItems]s, use
...
/// Typically, an enum is used. Each [DropDownItems] in [items] must be
...
///  * [DropDownItems], the class used to represent the [items].
...
class DropdownButton<T> extends StatefulWidget {
  ...
  /// must be equal to one of the [DropDownItems] values. If [items] or
  ...
  /// The value of the currently selected [DropDownItems].
  ///
  /// If [value] is null and the button is enabled, [hint] will be displayed
  /// if it is non-null.
  ///
  ...
  /// [DropDownItems]s in [items].
  ///
  /// When a [DropDownItems] is selected, the widget that will be displayed
  /// from the list corresponds to the [DropDownItems] of the same index
  /// in [items].
  ///
  /// {@tool dartpad}
  /// This sample shows a `DropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [DropDownItems].
  ///
  /// ** See code in examples/api/lib/material/dropdown/dropdown_button.selected_item_builder.0.dart **
  /// {@end-tool}
  ///
  /// If this callback is null, the [DropDownItems] from [items]
...
class _DropdownButtonState<T> extends State<DropdownButton<T>>
  ...
  void _updateSelectedIndex() {
    ...
    assert(
      widget.items!
              .where((DropDownItems<T> item) => item.value == widget.value)
              .length ==
          1,
    );
  ...
  Orientation _getOrientation(BuildContext context) {
    var result = MediaQuery.maybeOrientationOf(context);
    if (result == null) {
...
class DropdownButtonFormField<T> extends FormField<T> {
  ...
  /// [DropdownButton].
  DropdownButtonFormField({
    required List<DropDownItems<T>>? items,
    required this.onChanged,
    super.key,
    ...
    // DropdownButton.
        ...
        super(
          ...
          builder: (FormFieldState<T> field) {
            ...
            final showSelectedItem = items != null &&
                items
                    .where(
                      (DropDownItems<T> item) => item.value == state.value,
                    )
                    .isNotEmpty;
            ...
            final hasError = effectiveDecoration.errorText != null;
